...
"the password to use")
var flGitEmail = flag.String("email", envString("GIT_SYNC_EMAIL", "git-sync@git-sync"),
"the email to use for committing")
var flGitName = flag.String("name", envString("GIT_SYNC_NAME", "'Git Sync Sidecar'"),
"the name to use for committing")
...
func treeIsDirty(gitRoot string) bool {
    out, err := runCommand(path.Join(gitRoot, *flDest), "git","status","--porcelain")

    if err != nil{
        log.Errorf("error checking dirty tree : %s\n", err)
        return false
    }
...
func commitAndPush(gitRoot string, msg string) {
    out, err := runCommand(gitRoot, "git","add",".")

    if err != nil{
        log.Errorf("error adding changes: %s\n", err)
        log.Errorf("error adding changes: %s %s\n", err, out)
        return
    }

    //only changing environment for this one command, so manually exec

    commitMessage := fmt.Sprintf("'%s'", msg)
    cmd := exec.Command("git","commit","-a", "-m", commitMessage)
    cmd.Dir = gitRoot
    cmd.Env = append(os.Environ(),
            "GIT_AUTHOR_NAME=" + *flGitName,
            "GIT_AUTHOR_EMAIL=" + *flGitEmail,
            "GIT_COMMITTER_NAME=" + *flGitName,
            "GIT_COMMITTER_EMAIL=" + *flGitEmail,
        )

    log.V(0).Infof("cmd env:\n %v", cmd.Env)

    output, err := cmd.CombinedOutput()
    if err != nil{
        log.Errorf("error committing: %s %s\n", err, output)
        return
    }
